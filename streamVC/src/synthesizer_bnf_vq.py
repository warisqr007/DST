...
_quantizer = None # type: Quantizer
...
def load_models(config, ckpt, qckpt, device=None):
    """
    ...
    state_dict_q = load_checkpoint(qckpt, _device)
    _quantizer = Quantizer(hdim=_h.upsample_initial_channel, codebook_size=_h.vq_size).to(_device)
    _quantizer.load_state_dict(state_dict_q['quantizer'])
...
def process_wav(wav_file, ref_wav_file=None, out_file=None, sample_rate:int=16000):
    ...
    x_hat_quantized, indices, _ = _quantizer.vq(x_hat.transpose(1,2))
...
@torch.no_grad()
def process_wav_stream_with_spkr(wav_file, embed, out_file=None, sample_rate:int=16000, chunk_size:int=200):
    """
    Args:
        wav_file: Path to audio wav file
        sample_rate: target sample rate
    ...
    #Spkr Embed
    if embed is None or isinstance(embed, np.ndarray):
        raise ValueError("Speaker Embedding missing or not in correct format")
    spkr = torch.from_numpy(embed).to(_device).unsqueeze(0)
